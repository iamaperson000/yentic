...
const workflowSteps = [
  {
    label: '1. Create',
    description: 'Start with the opinionated HTML/CSS/JS starter or import your own repo via GitHub.'
  },
  {
    label: '2. Collaborate',
    description: 'Invite teammates into the same workspace and pair program in real time (multiplayer in progress).'
  },
  {
    label: '3. Ship',
    description: 'Deploy through your provider of choice with export-ready bundles and build logs.'
  }
];
    ...
    <>
      <section className="grid gap-10 md:grid-cols-[1.15fr_0.85fr] md:items-center">
        <div className="flex flex-col gap-6">
          ...
          <p className="text-lg text-white/70">
            ...
            thoughtful defaults. Yentic keeps the tooling out of your way so you can stay in flow.
          </p>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-emerald-500/10">
          ...
          <div className="space-y-3 text-sm text-white/70">
            {snippetLines.map((line, index) => (
              <p key={index} className="font-mono tracking-tight">{line}</p>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-white/10 bg-black/40 p-10">
        <div className="mb-10 max-w-3xl">
          ...
          <p className="mt-4 text-base text-white/60">
            We obsess over the little details: keyboard shortcuts that make sense, autosave that respects your intent,
            and environment defaults tuned for shipping web experiences fast.
          </p>
        </div>
      ...
      <section className="flex flex-col gap-6 rounded-3xl border border-white/10 bg-emerald-500/10 p-10">
        ...
        <p className="text-base text-white/70">
          Create your account with Google or email, invite collaborators, and let Yentic keep your projects synced
          across devices. Multiplayer editing, build logs, and workspace history are rolling out over the next few sprints.
        </p>
        <div className="flex flex-wrap gap-4">
          <Link
            href="/roadmap"
